...
#-----------------------------------
# Create CompAI virtual env
#-----------------------------------
python3 -m venv $JARVIS_DIR/$VIRTUAL_ENV

RESULT=$?
if  [ $RESULT -eq 0 ]; then
    echo "${green} CompAI virtual env creation succeeded! ${reset}"
else
    echo "${red} CompAI virtual env creation failed ${reset}"
    exit 1
fi
...
# Finished
#-----------------------------------
echo "${green} CompAI setup succeed! ${reset}"
echo "Start CompAI: bash run_jarvis.sh"
